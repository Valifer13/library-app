<div class="border border-gray-500 rounded-lg">
    ...
    <div class="p-3 grid gap-2">
        <p class="flex justify-between items-center"><a href="#" class="text-xs text-gray-600 dark:text-gray-400 hover:underline">{{ $book->author }}</a><a href="#" class="bg-blue-300 dark:bg-blue-400 hover:bg-blue-400 dark:hover:bg-blue-500 py-1 px-3 rounded-full text-sm">{{ $book->category->name }}</a></p>
        <h1 class="sm:text-base md:text-md min-h-14 font-medium">{{ Str::limit($book->title, 40) }}</h1>
        <p class="max-h-10 overflow-y-auto scrollbar-hide text-sm md:text-base" tabindex="-1">{{ Str::limit($book->description, 75) }}</p>
    </div>
